...
// chatServiceServer is implementation of pb.ChatServiceServer proto interface
...
func (s *chatServiceServer) Subscribe(e *empty.Empty, stream pb.ChatService_SubscribeServer) error {
	log.Print("Subscribe requested")
	for {
		m := <-s.msg
		n := pb.SubscribeResponse{Text: fmt.Sprintf("I have received from you: %s. Thanks!", m)}
